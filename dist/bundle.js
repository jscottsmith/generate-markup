...
function generateMarkup(containers, assets) {
    ...
    var stylesAndQueries = containers.reduce(function (acc, val) {
        ...
        var visibleAssets = assets.filter(function (asset) {
            return asset.containers.includes(val.id);
        }).reduce(function (_acc, _val) {
            return _acc + ('#asset-' + _val.id + ' { display: block; }\n');
        }, '');

        return acc + ('@media (min-width: ' + val.width + 'px) {\n                ' + visibleAssets + '\n            }\n');
    }, initialStyle);

    // set the within a style tag
    var style = '<style>' + stylesAndQueries + '</style>';

    // reduce assets to divs with id referencing their style
    var body = assets.reduce(function (acc, val) {
        return acc + ('<div id="asset-' + val.id + '"></div>');
    }, '');
...
var _generateMarkup = __webpack_require__(124);
...
// write the markup
var markup = (0, _generateMarkup2.default)(containers, assets);
